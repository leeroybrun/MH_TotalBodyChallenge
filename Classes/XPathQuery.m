...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult)
{
	...
	if (attribute)
	{
		...
		while (attribute)
		{
			...
			{
				[attributesArray addObject:attributeDictionary];
			}
		...
		{
			[resultForNode setObject:attributesArray forKey:@"nodeAttributeArray"];
        }
        
        if ([attributesDictionary count] > 0)
        {
            [resultForNode setObject:attributesDictionary forKey:@"nodeAttributeDictionary"];
        }
	}
    ...
    xmlBufferPtr contentXMLBuffer = xmlBufferCreate();
    int size = xmlNodeDump(contentXMLBuffer, currentNode->doc, currentNode, 0, 1);
    if(size > 0) {
        NSString *contentXML = [NSString stringWithCString:(const char *)contentXMLBuffer->content encoding:NSUTF8StringEncoding];
        [resultForNode setObject:contentXML forKey:@"nodeXMLContent"];
    }
